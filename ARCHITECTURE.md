...
- `map` — Google Maps embed URL (rendered in an `<iframe>` with a `title` from the `map_title` i18n string for accessibility)
...
layouts/
  ...
  _default/
    ...
    speaker-detail.html       # event/speaker detail page (back-to-events link + two-column .Content / sticky .event-info aside)
  events/
...
- **`baseof.html`** composes the page from `header` / `main` / `footer` blocks, and includes `site-style`, `site-meta`, and `schema` partials in `<head>`. Footer uses `partialCached` since it's identical across pages. For accessibility: `<html lang>` is set per language (`.Site.Language.Lang`), a `.skip-link` (`skip_to_content` i18n string) is the first focusable element, and the `main` block is wrapped in a `<main id="main">` landmark that the skip link targets.
...
1. **UI strings** — `i18n/es.toml` and `i18n/en.toml`, looked up in templates via `{{ T "key" }}` (e.g. `latest_event`, `join_cta`, `event_info_kicker`, `meta_title`, `meta_description`, `home_h1`, `home_lead`, `talk`, `talk_title_pending`, plus the accessibility strings `skip_to_content` and `map_title`). `about_speaker` uses a `%s` placeholder filled with the speaker name. The home hero `h1` (`home_h1`, keyword-focused) and lead paragraph (`home_lead`) are kept separate from the `meta_description` so the page heading and the meta tag can be optimized independently.
