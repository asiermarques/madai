...
- **`baseof.html`** composes the page from `header` / `main` / `footer` blocks, and includes `site-style`, `site-meta`, and `schema` partials in `<head>`. Footer uses `partialCached` since it's identical across pages. For accessibility: `<html lang>` is set per language (`.Site.Language.LanguageCode`), a `.skip-link` (`skip_to_content` i18n string) is the first focusable element, and the `main` block is wrapped in a `<main id="main">` landmark that the skip link targets.
