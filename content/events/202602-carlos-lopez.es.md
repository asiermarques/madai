+++
date = 2024-02-12T18:30:00Z
draft = true
layout = 'speaker-detail'
type = 'event'
title = 'No time to whisper'
speaker = 'Carlos López'
tagline = 'Staff Platform Engineer en DocPlanner'
day = '12.02.2026'
time = '19:00h'
where = 'Puerta de innovación, Madrid'
map = 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3038.06100241481!2d-3.7140335870457317!3d40.40749937132206!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd42277bae117a99%3A0x8b6111f0b83fbfbc!2sPuerta%20Innovaci%C3%B3n!5e0!3m2!1ses!2ses!4v1765662965334!5m2!1ses!2ses'
ticketsUrl = ''
description = 'Carlos López en MadAI - Staff Platform Engineer en DocPlanner'
image = 'img/speakers/carlos.jpg'
+++

{{<speaker_talk title="No time to whisper: lecciones aprendidas escalando un sistema GPU-powered AI para la transcripción de visitas médicas">}}
Crear IA para entornos clínicos deja muy poco margen para el error. En esta charla compartimos el recorrido de [Docplanner](https://www.docplanner.com/) al autoalojar y escalar un sistema de transcripción basado en GPUs para las visitas médicas, partiendo de un equipo con experiencia limitada en MLOps y en el uso de GPUs.

Repasaremos los retos reales a los que nos enfrentamos: desplegar y operar modelos de gran tamaño en producción, gestionar GPUs de forma eficiente, garantizar la fiabilidad bajo carga y cumplir con estrictos requisitos de cumplimiento en materia de datos médicos. Aprenderás las lecciones duras, las decisiones arquitectónicas y las estrategias prácticas de MLOps que nos permitieron pasar de los primeros fracasos a un sistema estable y de alta precisión en un entorno sanitario crítico.
{{</speaker_talk>}}

{{<speaker_bio title="Carlos López" tagline="Staff Platform Engineer en DocPlanner" image="img/speakers/carlos.jpg">}}
Carlos López Vecino es un ingeniero de plataforma con más de diez años de experiencia.  

Ahora mismo está enfocado en construir una plataforma de IA en Docplanner mientras ayuda a los equipos de producto a integrar IA en sus aplicaciones.

* [Linkedin](https://www.linkedin.com/in/carlos-lopez-vecino/)
* [Bluesky](https://bsky.app/profile/carloslopez.bsky.social)
{{</speaker_bio>}}